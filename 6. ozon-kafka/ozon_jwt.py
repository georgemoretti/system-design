...
def create_user(user: UserMongo, current_user: str = Depends(get_current_user)):
    user_dict = user.dict()
...
def search_users_by_name(
    first_name: str, last_name: str, current_user: str = Depends(get_current_user)
):
...
def add_to_cart(cart: Cart, db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
...
def get_cart(user_id: int, db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
